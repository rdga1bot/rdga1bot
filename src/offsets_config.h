...
// Navigation offsets (потребують калібровки через --calibrate)
// Знайти: повернутись на 90°, порівняти значення до/після
constexpr uintptr_t OFF_PLAYER_HEADING = 0x30;  // float radians або int16 — кут персонажа (HF default, непідтверджено)

// Name offset (wide char рядок або UTF-8)
// Знайти: шукати рядок "Goblin" або ім'я моба поряд через --calibrate dump
constexpr uintptr_t OFF_OBJ_NAME = 0x68;  // ptr → wide/char* (HF default, непідтверджено)
